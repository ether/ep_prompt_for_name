# Contributing to ep_prompt_for_name

Thanks for helping improve `ep_prompt_for_name` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_prompt_for_name` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_prompt_for_name from your local clone
pnpm run plugins i --path ../ep_prompt_for_name

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_prompt_for_name/` are picked up after a server restart.
...
* Etherpad version, ep_prompt_for_name version, browser + OS
* Steps to reproduce
