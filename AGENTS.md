# Agent Guide — ep_prompt_for_name

Prompt an author for their name 5 seconds after they type.
...
`ep_prompt_for_name` runs inside Etherpad's test harness. From an etherpad checkout that has installed this plugin via `pnpm run plugins i --path ../ep_prompt_for_name`:
